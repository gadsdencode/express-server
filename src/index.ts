...
// CORS setup
const allowedOrigins = ['https://kainbridge-mvp-gadsdencode-pro.vercel.app', 'https://kainbridge.vercel.app', 'https://kainbridge-mvp.vercel.app', 'https://kainbridge.com/coaching/', 'http://localhost:3000', 'https://kainbridge.com/', 'https://kainbridge.com/app/', 'https://kainbridge.com/coaching-2/', 'https://kainbridge.com/coach-signup/'];
app.use(cors({
...
api.get('/fetch-corresponding-user-notes', async (req: Request, res: Response) => {
  const { userId, relatedUserName } = req.query as { userId?: string, relatedUserName?: string };
...
api.get('/coach-user-relationships', async (req: Request, res: Response) => {
  ...
  api.get('/search-users-filtered-notes', async (req, res) => {
    const { userId, query } = req.query;  // `query` now expected to be part of a name to search for
  
    if (!userId) return res.status(400).json({ message: 'User ID is required' });
    if (!query) return res.status(400).json({ message: 'Search query is required' });
  
    try {
      // Fetch user IDs that this coach can access
      ...
      if (userCoachRelationships.data.length === 0) return res.status(404).json({ message: 'No related users found for this coach.' });
  
      const userIds = userCoachRelationships.data.map(relationship => relationship.user_id);
  
      // Fetch profiles where the name matches the query and the ID is in the list of user IDs this coach can access
